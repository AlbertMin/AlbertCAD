...
namespace Albert.Geometry.External
{
    ...
    /// 
    /// </summary>
    public class InsideRegionAlgorithm
    {
        /// <summary>
        ...
        {
            Vector2D v2 = Transform.TransformUtil.Projection(vector3D);
            List<Line2D> outs = Transform.TransformUtil.Projection(outLines);
            return Check(v2, outs);
        }
        /// <summary>
        /// 0表示点在边上，1表示在内部，-1表示在外部
        /// </summary>
        /// <param name="vector3D"></param>
        /// <param name="outLines">投影到xoy平面后的楼板外轮廓</param>
        /// <returns></returns>
        public int Check(Vector2D vector3D, List<Line2D> outLines)
        {
            int count = 0;//交点的个数

            double x = vector3D.X;
            double y = vector3D.Y;
            foreach (Line2D line in outLines)
            {
                ...
                if (nflag < 0) return 0;//出现-1，表明点在边上
                count += nflag;
            }
