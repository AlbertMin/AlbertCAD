...
namespace Albert.Geometry.External
{
    public class GraphicAlgorithm
    {
        ...
        public static List<Vector2D> GetAllPointOfGivenLineExtension(List<Line2D> lines, Vector2D XVector, Vector2D YVector)
        {
            ...
            {
                Line2D line1 = Line2D.Create(point, XVector);
                Line2D line2 = Line2D.Create(point, YVector);
                tempLines.Add(line1);
                tempLines.Add(line2);
            }
        ...
        /// <param name="angle"></param>
        /// <returns></returns>
        public static Line2D LineCoordTransform(Line2D line,double angle)
        {
            Vector2D start = new Vector2D(line.Start.X * Math.Cos(angle) - line.Start.Y * Math.Sin(angle),
                        line.Start.X * Math.Sin(angle) + line.Start.Y * Math.Cos(angle));

            Vector2D end = new Vector2D(line.End.X * Math.Cos(angle) - line.End.Y * Math.Sin(angle),
                        line.End.X * Math.Sin(angle) + line.End.Y * Math.Cos(angle));
        ...
        public static bool IsContainOtheRegion(List<Line3D> lines, Line3D compare)
        {
            ...
            if (IsCoplanarity(lines, compare))
            {
                ...
                //进行坐标转换
                //   TransformUtil coordinateAlgorithm1 = new TransformUtil(copyLines);
                //进行了坐标转换
                // Matrix4 matrix = coordinateAlgorithm1.TransformXY();
                //对当前线段也进行坐标转换
                // coordinateAlgorithm1.Transform(copyCompare, matrix);
                //转换为Line2D
                List<Line2D> line2ds = TransformUtil.Projection(copyLines);
                //转化当前线段
                Line2D compareline2d = TransformUtil.Projection(copyCompare);
        ...
        /// <param name="f2"></param>
        /// <returns></returns>
        public static bool IsCoincidenceWith(Line3D l1, Polygon3D f2)
        {
        ...
        /// <param name="lines"></param>
        /// <returns></returns>
        public static Vector2D GetCentrality(List<Line2D> lines)
        {
            ...
            center = new Vector2D(cxs.Average(), cys.Average());
            return center;
        }
        /// <summary>
        /// 获取任意多边形的面积
        ...
        {
            //找到最大封闭区域，也就是房间的区域
            List<Line3D> closeLines = GraphicAlgorithm.ClosedLookup(lines, true, true).FirstOrDefault();
            //对线段进行融合
        ...
        /// <summary>
        /// 对一个连续的连段进行前后点排序,direction为正则为逆时针，否则为顺时针
        /// </summary>
        /// <param name="lines"></param>
        /// <returns></returns>
        public static List<Line2D> Sort(List<Line2D> lines, Vector2D startp = null, int direction = 0)
        {
            SortLinesAlgorithm sort = new SortLinesAlgorithm();
            if (startp == null && direction == 0)
            {
                return sort.Sort(lines);
            }
            else if (startp != null && direction == 0)
            {

                return sort.SortByPoint(lines, startp);
            }
            else if (startp == null && direction != 0)
            {

                return sort.SortByDirection(lines, direction);
            }
            else
            {
                return sort.SortByDirection(lines, startp, direction);
            }

        }

        /// <summary>
        /// 对当前传入的线段，按照指定起点或者指定方向进行排序,direction为右手法则，大拇指指定的方向
        /// </summary>
        /// <param name="lines"></param>
        /// <param name="startP"></param>
        /// <param name="direction"></param>
        /// <returns></returns>
        public static List<Line3D> Sort(List<Line3D> lines, Vector3D startP=null, Vector3D direction = null)
        {
            SortLinesAlgorithm sort = new SortLinesAlgorithm();
            if (direction == null && startP == null)
            {
                return sort.Sort(lines);
            }
            else if (startP != null && direction == null)
            {
                return sort.SortByPoint(lines, startP);
            }
            else if (startP == null && direction != null)
        ...
        /// <returns></returns>
        public static List<Line2D> Elastic(List<Line2D> region, double range, List<Line2D> neverOffset = null)
        {

            ElasticityAlgorithm elasticity = new ElasticityAlgorithm();
            //进行缩放和扩张
            var elasticityLines = elasticity.Elastic(region, range, neverOffset);
            //返回结果图形
            return elasticityLines;
        }
        ...
        /// <param name="range"></param>
        /// <returns></returns>
        public static List<Line3D> Elastic(List<Line3D> region, double range)
        {

            ElasticityAlgorithm elasticity = new ElasticityAlgorithm();
            //进行缩放和扩张
            var elasticityLines = elasticity.Elastic(region, range,null);
            //返回结果图形
            return elasticityLines;
        }
        ...
        /// <returns></returns>
        public static List<Line2D> Elastic(List<Tuple<Line2D, double>> offsets)
        {
            var nlist = new List<Tuple<Line2D, double>>(offsets);
            ElasticityAlgorithm elasticity = new ElasticityAlgorithm();
            var resultList = elasticity.Elastic2(offsets);
            return resultList;
        }
        /// <summary>
        /// 对指定的二维线段进行打断
        ...
        /// <summary>
        /// 判断一个点是否在一个多边形的内部,0表示点在边上，1表示在内部，-1表示在外部
        /// </summary>
        /// <param name="v"></param>
        /// <param name="searchLines"></param>
        /// <returns></returns>
        public static int InsideOfRegion(Vector2D v, List<Line2D> searchLines)
        {
            InsideRegionAlgorithm Inside = new InsideRegionAlgorithm();
            return Inside.Check(v, searchLines);
        }
        ...
        /// <param name="searchLines"></param>
        /// <returns></returns>
        public static int InsideOfRegion(Vector3D v, List<Line3D> searchLines)
        {
            InsideRegionAlgorithm Inside = new InsideRegionAlgorithm();
            return Inside.Check(v, searchLines);
        }
        ...
        /// <param name="lines"></param>
        /// <returns></returns>
        public static List<Line2D> Merge(List<Line2D> lines) {
            MergeLinesAlgorithm ma = new MergeLinesAlgorithm();
            return ma.Merge(lines);
        }
        ...
        /// <param name="lines"></param>
        /// <returns></returns>
        public static List<Line3D> Merge(List<Line3D> lines)
        {
            MergeLinesAlgorithm ma = new MergeLinesAlgorithm();
            return ma.Merge(lines);
        }
        ...
        /// <summary>
        /// 将一个矩形，按照指定的尺寸大小，铺满当前矩形
        /// </summary>
        /// <param name="rectangle"></param>
        /// <param name="w"></param>
        /// <param name="h"></param>
        /// <returns></returns>
        public static List<List<Line2D>> PartitionRectangle(List<Line2D> rectangle, double w, double h,Vector2D offset=null) {

            PartitionRectanglesAlgorithm partitionRectanglesAlgorithm = new PartitionRectanglesAlgorithm();
            return partitionRectanglesAlgorithm.Partition(rectangle, w, h, offset);
        }
        ...
        /// 用于比较两个多边形是否相等
        /// </summary>
        /// <param name="l1s"></param>
        /// <param name="l2s"></param>
        /// <returns></returns>
        public static bool CompareLines(List<Line2D> l1s, List<Line2D> l2s) {

            ComparisonPolygonAlgorithm comparison = new ComparisonPolygonAlgorithm();
            return comparison.Compare(l1s, l2s);
        }
        ...
        /// <param name="l2s"></param>
        /// <returns></returns>
        public static bool CompareLines(List<Line3D> l1s, List<Line3D> l2s)
        {
            var l1 = TransformUtil.Projection(l1s);
            var l2 = TransformUtil.Projection(l2s);
            ComparisonPolygonAlgorithm comparison = new ComparisonPolygonAlgorithm();
            return comparison.Compare(l1, l2);
        }
        /// <summary>
        /// 用于处理线和多边形的关系，返回值为1，说明线在多边形内部，-1说明在外部，0则说明相交，且返回在内部的线段
        /// </summary>
        /// <param name="line"></param>
        /// <param name="outerlines"></param>
        /// <param name="innerlines"></param>
        /// <returns></returns>
        public static int LineInsideOfRegion(Line2D line, List<Line2D> outerlines, List<Line2D> innerlines)
        {
            LineInsideRegionAlgorithm lineInsideRegionAlgorithm = new LineInsideRegionAlgorithm();
            List<Line2D> inlines = null;
            int result = lineInsideRegionAlgorithm.Check(line, outerlines, out inlines);

            if (innerlines != null && inlines != null)
            {
                innerlines.AddRange(inlines);
            }
            return result;
        }
        ...
        /// <param name="innerlines"></param>
        /// <returns></returns>
        public static int PolygonInsideOfRegion(List<Line2D> lines, List<Line2D> outerlines, List<List<Line2D>> innerlines)
        {
            PolygonInsideRegionAlgorithm polygonInsideRegionAlgorithm = new PolygonInsideRegionAlgorithm();
            List<List<Line2D>> inlines = null;
            int result = polygonInsideRegionAlgorithm.Check(lines, outerlines, out inlines);
